...
int main(int argc, char *argv[]) {
  ...
  l_source_array = (int *)malloc(src_size * sizeof(int));
  ...
  int *l_source_array = 0;
  l_source_array = (int *)malloc(dest_size * sizeof(int));

  int x_0, x_1, x_2, x_3, x_5, x_6, x_7, x_8, sum_0, sum_1;
  for (row = 1; row < (information.height + 1); row++) {
    l_source_array[row - 1] = (int *)malloc(information.width * sizeof(int));
...
for (row = 0; row < result_row_size; row++) {
  newData[row] = (int *)malloc(result_column_size * sizeof(int));
